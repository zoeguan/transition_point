...
p3 = qplot(sigma.vals, log(ratio$ridge), size=I(0.6), xlab=expression(sigma^2), 
...
p2 = qplot(sigma.vals, log(ratio$lasso), size=I(0.6), xlab=expression(sigma^2), 
           ylab=expression( paste("log", bgroup("(", frac( "MSPE"["CSL"], "MSPE"["Merged"]), ")" ) ))) +
  geom_hline(yintercept=0, color='blue', linetype = "longdash") +
  ggtitle("LASSO") +
...
p4 = qplot(sigma.vals, log(ratio$nn), size=I(0.6), xlab=expression(sigma^2), 
           ylab=expression( paste("log", bgroup("(", frac( "MSPE"["CSL"], "MSPE"["Merged"]), ")" ) ))) +
  geom_hline(yintercept=0, color='blue', linetype = "longdash") +
...
p5 = qplot(sigma.vals, log(ratio$rf), size=I(0.6), xlab=expression(sigma^2), 
           ylab=expression( paste("log", bgroup("(", frac( "MSPE"["CSL"], "MSPE"["Merged"]), ")" ) ))) + 
