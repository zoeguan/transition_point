...
results.file = "transition_point_thm1_negcorr.RData"

if (length(args)>=1) {
  results.file = as.character(args[1])
}
...
p1 = qplot(sigma.vals, log(ratio$csl), size=I(0.8), xlab=expression(bar(sigma^2)), 
  ...
  geom_vline(xintercept=sigma2_star*conversion.ratio, color='green', linetype = "longdash") +
...
p3 = qplot(sigma.vals, log(ratio$ridge), size=I(0.8), xlab=expression(bar(sigma^2)), 
           ylab=expression( paste("log", bgroup("(", frac( "MSPE"["Ensemble"], "MSPE"["Merged"]), ")" ) ))) + 
  geom_vline(xintercept=sigma2_star.ridge*conversion.ratio, color='green', linetype = "longdash") +
  geom_hline(yintercept=0, color='blue', linetype = "longdash") +
...
if (results.file %in% "transition_point_thm1_negcorr.RData") {
  png('plots_thm1_negcorr.png', width=600, height=600, res=100)
  grid.arrange(p1, p3, p2, p4, p5, ncol=2)
  dev.off()
} else {
  png('plots_thm1_poscorr.png', width=600, height=600, res=100)
  grid.arrange(p1, p3, p2, p4, p5, ncol=2)
  dev.off()
}
