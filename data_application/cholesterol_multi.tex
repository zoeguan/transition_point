...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{meta.exp} \hlkwb{=} \hlstd{meta.exp[,} \hlkwd{which}\hlstd{(}\hlkwd{colSums}\hlstd{(meta.exp)}\hlopt{!=}\hlnum{0}\hlstd{)]}
\hlcom{# remove features that are very sparse in the training data}
\hlstd{min.samples} \hlkwb{=} \hlkwd{floor}\hlstd{(}\hlkwd{length}\hlstd{(}\hlkwd{which}\hlstd{(meta.merged}\hlopt{$}\hlstd{studyID}\hlopt{==}\hlstr{"KarlssonFH_2013.marker_abundance.stool"} \hlopt{&}
                                   \hlstd{meta.merged}\hlopt{$}\hlstd{gender}\hlopt{==}\hlstr{"female"}\hlstd{))}\hlopt{/}\hlnum{10}\hlstd{)}
\hlstd{meta.exp} \hlkwb{=} \hlstd{meta.exp[}\hlkwd{vapply}\hlstd{(meta.exp,} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{)}
  \hlkwd{length}\hlstd{(}\hlkwd{unique}\hlstd{(x[}\hlkwd{which}\hlstd{(meta.merged}\hlopt{$}\hlstd{studyID}\hlopt{==}\hlstr{"KarlssonFH_2013.marker_abundance.stool"} \hlopt{&}
                          \hlstd{meta.merged}\hlopt{$}\hlstd{gender}\hlopt{==}\hlstr{"female"}\hlstd{)]))}\hlopt{>}\hlstd{min.samples} \hlopt{&}
    \hlkwd{length}\hlstd{(}\hlkwd{unique}\hlstd{(x[}\hlkwd{which}\hlstd{(meta.merged}\hlopt{$}\hlstd{studyID}\hlopt{==}\hlstr{"QinJ_2012.marker_abundance.stool"} \hlopt{&}
                            \hlstd{meta.merged}\hlopt{$}\hlstd{gender}\hlopt{==}\hlstr{"female"}\hlstd{)]))}\hlopt{>}\hlstd{min.samples,} \hlkwd{logical}\hlstd{(}\hlnum{1L}\hlstd{))]}
...
\hlstd{meta.exp}\hlopt{$}\hlstd{group} \hlkwb{=} \hlkwd{as.numeric}\hlstd{(}\hlkwd{as.factor}\hlstd{(meta.merged}\hlopt{$}\hlstd{studyID))}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{meta.train} \hlkwb{=} \hlstd{meta.exp[}\hlkwd{which}\hlstd{(meta.exp}\hlopt{$}\hlstd{group} \hlopt{%in%} \hlnum{2}\hlopt{:}\hlnum{3}\hlstd{),]}
\hlstd{meta.test} \hlkwb{=} \hlstd{meta.exp[}\hlkwd{which}\hlstd{(meta.exp}\hlopt{$}\hlstd{group} \hlopt{%in%} \hlnum{1}\hlstd{),]}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{feature.list} \hlkwb{=} \hlkwd{lapply}\hlstd{(}\hlkwd{split}\hlstd{(}\hlkwd{as.list}\hlstd{(}\hlkwd{as.data.frame}\hlstd{(meta.train[,} \hlkwd{which}\hlstd{(}\hlopt{!}\hlkwd{names}\hlstd{(meta.train)} \hlopt{%in%}
                                                                       \hlkwd{c}\hlstd{(}\hlstr{"cholesterol"}\hlstd{,} \hlstr{"age"}\hlstd{,}
                                                                         \hlstr{"group"}\hlstd{))])),}
                            \hlkwd{cut}\hlstd{(}\hlnum{1}\hlopt{:}\hlkwd{ncol}\hlstd{(meta.train[,} \hlkwd{which}\hlstd{(}\hlopt{!}\hlkwd{names}\hlstd{(meta.train)} \hlopt{%in%}
                                                            \hlkwd{c}\hlstd{(}\hlstr{"cholesterol"}\hlstd{,} \hlstr{"age"}\hlstd{,} \hlstr{"group"}\hlstd{))]),} \hlnum{20}\hlstd{)),}
                      \hlstd{as.data.frame)}
\hlstd{weight.list} \hlkwb{=} \hlkwd{lapply}\hlstd{(feature.list,} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{)} \hlkwd{linear.correlation}\hlstd{(meta.train}\hlopt{$}\hlstd{cholesterol} \hlopt{~}\hlstd{., x))}
...
\hlstd{max.features} \hlkwb{=} \hlnum{20}
\hlstd{meta.train} \hlkwb{=} \hlstd{meta.train[,} \hlkwd{which}\hlstd{(}\hlkwd{names}\hlstd{(meta.train)} \hlopt{%in%} \hlkwd{c}\hlstd{(weight.df}\hlopt{$}\hlstd{feature[}\hlnum{1}\hlopt{:}\hlstd{max.features],}
                                                         \hlkwd{c}\hlstd{(}\hlstr{"cholesterol"}\hlstd{,} \hlstr{"age"}\hlstd{,} \hlstr{"group"}\hlstd{)))]}
\hlstd{meta.test} \hlkwb{=} \hlstd{meta.test[,} \hlkwd{which}\hlstd{(}\hlkwd{names}\hlstd{(meta.test)} \hlopt{%in%} \hlkwd{c}\hlstd{(weight.df}\hlopt{$}\hlstd{feature[}\hlnum{1}\hlopt{:}\hlstd{max.features],}
                                                      \hlkwd{c}\hlstd{(}\hlstr{"cholesterol"}\hlstd{,} \hlstr{"age"}\hlstd{,} \hlstr{"group"}\hlstd{)))]}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{ndat} \hlkwb{=} \hlnum{2}
\hlstd{meta.all} \hlkwb{=} \hlkwd{rbind}\hlstd{(meta.train, meta.test)}
\hlstd{parts} \hlkwb{=} \hlkwd{split}\hlstd{(meta.all, meta.all}\hlopt{$}\hlstd{group)}
\hlstd{edat_train} \hlkwb{=} \hlstd{parts[}\hlnum{2}\hlopt{:}\hlnum{3}\hlstd{]}
\hlstd{edat_test} \hlkwb{=} \hlstd{parts[}\hlnum{1}\hlstd{]}
...
\hlstd{parts2} \hlkwb{=} \hlkwd{split}\hlstd{(}\hlkwd{cbind}\hlstd{(}\hlkwd{rep}\hlstd{(}\hlnum{1}\hlstd{,} \hlkwd{nrow}\hlstd{(meta.all)), meta.all[, feature.cols]), meta.all}\hlopt{$}\hlstd{group)}
\hlstd{edat_train2} \hlkwb{=} \hlstd{parts2[}\hlnum{2}\hlopt{:}\hlnum{3}\hlstd{]}
\hlstd{edat_test2} \hlkwb{=} \hlstd{parts2[}\hlnum{1}\hlstd{]}
...
\end{alltt}
\begin{verbatim}
## [1]  0.001405357 19.564912992
\end{verbatim}
\begin{alltt}
\hlstd{sigma2.bar} \hlopt{>} \hlstd{ls.bounds[}\hlnum{2}\hlstd{]}
\end{alltt}
\begin{verbatim}
## [1] TRUE
\end{verbatim}
\begin{alltt}
\hlstd{sigma2.bar} \hlopt{>} \hlstd{ridge.bounds[}\hlnum{2}\hlstd{]}
\end{alltt}
\begin{verbatim}
## [1] TRUE
\end{verbatim}
...
\hlstd{\}}
\hlkwd{save}\hlstd{(err.ridge, err.ols,} \hlkwc{file}\hlstd{=}\hlstr{"cholesterol_multi.RData"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
